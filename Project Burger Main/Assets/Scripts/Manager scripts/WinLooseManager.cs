...
public class WinLooseManager : MonoBehaviour {
    ...
    void GoldWinCheck() {//Gold Check Each Time Gold Changes
        if(LevelManager.Instance.ScoreManager.GoldEarned - LevelManager.Instance.ScoreManager.GoldLost >= GoldWinAmount) {
            GameEnd();
        }
    }
